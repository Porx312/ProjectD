...
function Comments({ trackId }: { trackId: Id<"tracks"> }) {
  const { user } = useUser()
  const [isSubmitting, setIsSubmitting] = useState(false)
  const [deletingCommentId, setDeletingCommentId] = useState<string | null>(null)

  const comments = useQuery(api.tracks.getComments, { trackId }) || []
  const addComment = useMutation(api.tracks.addComment)
  const deleteComment = useMutation(api.tracks.deleteComment)

  const handleSubmitComment = async (content: string) => {
    setIsSubmitting(true)

    try {
      await addComment({ trackId, content })
  ...
  const handleDeleteComment = async (commentId: Id<"trackComments">) => {
